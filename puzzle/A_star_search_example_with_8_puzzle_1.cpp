...
///::::::::::::::::::::::The heuristic is based on City Block Distance::::::::::::
#include<bits/stdc++.h>
#define MX 5
#define mp make_pair
...
int x[]={3, 1, 1, 1, 2, 2, 2, 3, 3};
int y[]={3, 1, 2, 3, 1, 2, 3, 1, 2};
...
int block_distance(int state[][MX])
{
    int cnt=0;
    for(int i=1; i<=3; i++)
    {
        for(int j=1; j<=3; j++)
        {
            ...
            int temp = (abs(i-x[state[i][j]])+abs(j-y[state[i][j]]));
            cnt+=temp;
        }
    }
    return cnt;
...
void solve(int initial[][MX])
{
    ...
    int h=block_distance(initial), explored=0;
    ...
    while(true)
    {
        ...
        if(a>=1 && a<=3 && b>=1 && b<=3)
        {
            ...
            {
                int hn=block_distance(cur_state);
